    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-7xl px-6 py-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <input className="border border-border rounded-sm bg-card px-4 py-2 text-sm outline-none placeholder:text-muted-foreground focus:border-[var(--olive)] transition w-full sm:w-80" placeholder="Buscar..." value={search} onChange={(e) => setSearch(e.target.value)} />
          <button onClick={openCreate} className="flex items-center gap-2 bg-[var(--olive)] text-primary-foreground px-5 py-2.5 rounded-sm hover:bg-[var(--olive-deep)] transition text-sm font-medium whitespace-nowrap">
        ...
        <div className="overflow-x-auto rounded-sm border border-border shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-[var(--olive-deep)] text-cream">
              <tr>
                {columns.map((col) => (<th key={col.key} className="text-left px-5 py-3.5 font-medium tracking-wide text-xs uppercase whitespace-nowrap">{col.label}</th>))}
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((item, idx) => (
                  <tr key={item.id} className={idx % 2 === 0 ? "bg-card" : "bg-background"}>
                    {columns.map((col) => (<td key={col.key} className="px-5 py-3.5 align-middle">{formatCell ? (formatCell(col.key, item) ?? item[col.key]) : item[col.key]}</td>))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-background rounded-sm shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              {fields.map((f) => (
                <div key={f.key}>
                  <label className="block text-sm font-medium mb-1.5">{f.label}{f.required && <span className="text-red-500 ml-1">*</span>}</label>
                  {f.type === "select" ? (
                    <select className="w-full border border-border rounded-sm bg-card px-3 py-2 text-sm outline-none focus:border-[var(--olive)] transition" value={form[f.key] ?? ""} required={f.required} onChange={(e) => setForm((p) => ({ ...p, [f.key]: e.target.value }))}>
                      <option value="">Selecione...</option>
                      {f.options.map((opt) => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                    </select>
                  ) : (
                    <input type={f.type || "text"} className="w-full border border-border rounded-sm bg-card px-3 py-2 text-sm outline-none focus:border-[var(--olive)] transition" value={form[f.key] ?? ""} required={f.required} onChange={(e) => setForm((p) => ({ ...p, [f.key]: e.target.value }))} />
                  )}
                </div>
              ))}
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" className="px-5 py-2 rounded-sm bg-[var(--olive)] text-primary-foreground text-sm font-medium hover:bg-[var(--olive-deep)] transition">{modal.mode === "create" ? "Criar" : "Salvar"}</button>
              </div>
            </form>
          </div>
        </div>
